	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (user != "" && !dao.exist_usuario(user)) {
			...
			while (em.hasMoreElements()) {
				...
				// total_votos = calculos.total_votos(req.getParameterNames(),
				// req.getParameter(paraName));
				// if(req.getParameter(paraName) != "" && paraName.indexOf("PP")
				// != -1 ){
			...
			// System.out.println(calc.calcula_esca(req.getParameterNames(),
			// datos2));
			for (int w = 0; w < 10; w++) {
				// System.out.println("Total escaños " +
				// calc.metodo_dhont_Avila(req.getParameterNames(), datos2)[w]);
			...
			// String votos =
			// String.valueOf(calc.total_votos(req.getParameterNames(),
			// datos2));

			// List<Partido> votosTabla =
			...
			// calc.calcula_esc(calc.crea_mapa(req.getParameterNames(),
			// datos2));
